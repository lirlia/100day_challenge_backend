...
func TestWebUIIntegrationDataOperations(t *testing.T) {
	...
	for _, data := range testData {
		...
		resp, err := http.NewRequest("PUT", baseURL+"/api/chord/data", bytes.NewBuffer(jsonBody))
		if err != nil {
	...
	// 3. 存在しないキーの検索テスト
	t.Log("3. 存在しないキーの検索テスト")
	resp, err := http.Get(baseURL + "/api/chord/data?key=nonexistent_key")
	if err != nil {
		t.Fatalf("GET /api/chord/data failed for nonexistent key: %v", err)
	}
	defer resp.Body.Close()

	var getResponse map[string]interface{}
	if err := json.NewDecoder(resp.Body).Decode(&getResponse); err != nil {
		t.Fatalf("Failed to decode GET response for nonexistent key: %v", err)
	}
	...
	// 4. データ削除テスト
	t.Log("4. データ削除テスト")
	deleteKey := testData[0].key

	req, err := http.NewRequest("DELETE", fmt.Sprintf("%s/api/chord/data?key=%s", baseURL, deleteKey), nil)
	if err != nil {
		t.Fatalf("Failed to create DELETE request for %s: %v", deleteKey, err)
	}

	client := &http.Client{}
	resp, err = client.Do(req)
	if err != nil {
		t.Fatalf("DELETE /api/chord/data failed for %s: %v", deleteKey, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		t.Errorf("Expected status 200 for DELETE %s, got %d", deleteKey, resp.StatusCode)
	}

	var deleteResponse map[string]interface{}
	if err := json.NewDecoder(resp.Body).Decode(&deleteResponse); err != nil {
		t.Fatalf("Failed to decode DELETE response for %s: %v", deleteKey, err)
	}

	if !deleteResponse["success"].(bool) {
		t.Errorf("Data deletion failed for key %s", deleteKey)
	}

	t.Log("データ操作API テスト完了")
}

// Web UI統合テスト: パフォーマンスとレスポンス時間
func TestWebUIIntegrationPerformance(t *testing.T) {
	t.Log("=== Web UI統合テスト: パフォーマンスとレスポンス時間 ===")
	...
	// ノード追加
	for i := 0; i < 5; i++ {
		reqBody := map[string]string{"address": fmt.Sprintf("perf-node-%d.local:800%d", i, i)}
		jsonBody, _ := json.Marshal(reqBody)

		resp, err := http.Post(baseURL+"/api/chord", "application/json", bytes.NewBuffer(jsonBody))
		if err != nil {
			t.Fatalf("Failed to add performance test node %d: %v", i, err)
		}
		resp.Body.Close()
	}
	...
	// 1. GET /api/chord のレスポンス時間測定
	t.Log("1. リング状態取得のレスポンス時間測定")
	getRingTimes := make([]time.Duration, requestCount)

	for i := 0; i < requestCount; i++ {
		start := time.Now()
		resp, err := http.Get(baseURL + "/api/chord")
		if err != nil {
			t.Fatalf("GET /api/chord failed in performance test: %v", err)
		}
		...
		getRingTimes[i] = time.Since(start)
	}

	avgGetRingTime := calculateAverageTime(getRingTimes)
	t.Logf("GET /api/chord 平均レスポンス時間: %v", avgGetRingTime)

	// 2. データ操作のレスポンス時間測定
	t.Log("2. データ操作のレスポンス時間測定")
	putTimes := make([]time.Duration, requestCount)
	getTimes := make([]time.Duration, requestCount)
	...
	for i := 0; i < requestCount; i++ {
		...
		start := time.Now()
		req, _ := http.NewRequest("PUT", baseURL+"/api/chord/data", bytes.NewBuffer(jsonBody))
		req.Header.Set("Content-Type", "application/json")

		client := &http.Client{}
		resp, err := client.Do(req)
		if err != nil {
			t.Fatalf("PUT failed in performance test: %v", err)
		}

		io.ReadAll(resp.Body)
		resp.Body.Close()
		putTimes[i] = time.Since(start)

		// GET操作の時間測定
		start = time.Now()
		resp, err = http.Get(fmt.Sprintf("%s/api/chord/data?key=%s", baseURL, key))
		if err != nil {
			t.Fatalf("GET failed in performance test: %v", err)
		}
	...
	avgPutTime := calculateAverageTime(putTimes)
	avgGetTime := calculateAverageTime(getTimes)

	t.Logf("PUT /api/chord/data 平均レスポンス時間: %v", avgPutTime)
	t.Logf("GET /api/chord/data 平均レスポンス時間: %v", avgGetTime)

	// パフォーマンス要件の検証
	maxAcceptableTime := time.Millisecond * 100 // 100ms以下

	if avgGetRingTime > maxAcceptableTime {
		t.Errorf("GET /api/chord のレスポンス時間が遅すぎます: %v > %v", avgGetRingTime, maxAcceptableTime)
	}
	if avgPutTime > maxAcceptableTime {
		t.Errorf("PUT /api/chord/data のレスポンス時間が遅すぎます: %v > %v", avgPutTime, maxAcceptableTime)
	}
	if avgGetTime > maxAcceptableTime {
		t.Errorf("GET /api/chord/data のレスポンス時間が遅すぎます: %v > %v", avgGetTime, maxAcceptableTime)
	}
