        ...
        <div className="space-y-4">
          <div className="flex items-center">
            <input
              ...
              disabled={isApplying}
            />
            <label htmlFor="nosniffEnabled" className="ml-2 block text-base text-gray-300">
      ...
      <div className="mt-8 p-6 bg-gray-800 rounded-lg shadow-md">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="border border-dashed border-gray-600 rounded-md p-4">
            <h5 className="text-lg font-medium mb-2 text-gray-300">テスト1: Content-Type: image/jpeg だが中身はHTML</h5>
            <p className="text-base text-gray-500 mb-2">サーバーは <code>Content-Type: image/jpeg</code> と共にHTML文字列 (<code>&lt;h1&gt;HTMLです&lt;/h1&gt;</code>) を返します。</p>
            <iframe
          ...
          <div className="border border-dashed border-gray-600 rounded-md p-4">
            <h5 className="text-lg font-medium mb-2 text-gray-300">テスト2: Content-Type: text/plain だが中身はJavaScript</h5>
            <p className="text-base text-gray-500 mb-2">サーバーは <code>Content-Type: text/plain</code> と共にJavaScriptコード (<code>alert('Script executed as text!')</code>) を返します。</p>
            ...
            ></iframe>
            <p className="text-base text-gray-500 mt-2">
              <strong>nosniff 無効時:</strong> ブラウザが内容をスクリプトとスニッフィングし、実行する可能性があります (ブラウザとiframeのsandbox設定依存)。<br />
              <strong>nosniff 有効時:</strong> ブラウザはContent-Typeを信じ、プレーンテキストとして表示するか、スクリプトとしては実行しないはずです。
              （コンソールでエラーが出るか確認してください）
            </p>
          </div>
        </div>
        <p className="text-base text-gray-500 mt-4">注意: 実際のMIMEスニッフィングの挙動はブラウザの種類やバージョンによって異なる場合があります。</p>
      </div>
    </div>
  );
}
