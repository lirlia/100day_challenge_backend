...
sequenceDiagram
    ...
    loop ポーリング
        新デバイス->>サーバー: 15. ステータスポーリング
        サーバー->>新デバイス: 現在のステータス返却
    end
    
    Note over 既存デバイス: 16. 通常ログイン・ダッシュボード表示
    既存デバイス->>サーバー: 17. 承認リクエスト一覧取得
    サーバー->>既存デバイス: 18. 保留中のリクエスト返却
    既存デバイス->>サーバー: 19. 承認プロセス開始
    サーバー->>DB: 20. チャレンジ生成・保存
    サーバー->>既存デバイス: 21. 認証オプション返却
    Note over 既存デバイス: 22. WebAuthn API呼び出し<br>(navigator.credentials.get)
    既存デバイス->>サーバー: 23. 認証応答送信
    サーバー->>サーバー: 24. 認証応答検証
    サーバー->>DB: 25. リクエストを承認済みに更新
    サーバー->>既存デバイス: 26. 承認成功返却
    
    Note over 新デバイス: 27. 次回ポーリングで「承認済み」検出
    Note over 新デバイス: 28. ログイン完了・ダッシュボードへリダイレクト
```
