...
sequenceDiagram
    ...
    クライアント->>UA: 4. WebAuthn API呼び出し<br>(navigator.credentials.create)
    Note over UA: 5. ユーザーに認証方法選択を促す<br>(指紋/顔認証/PIN等)
    UA-->>UA: 6. 秘密鍵/公開鍵ペア生成
    Note over UA: 7. 秘密鍵はデバイスの<br>セキュアエリアに保存
    UA->>クライアント: 8. 公開鍵と認証情報を返却
    
    クライアント->>サーバー: 9. 公開鍵と認証情報送信
    サーバー->>サーバー: 10. 認証情報検証
    サーバー->>DB: 11. ユーザー作成
    サーバー->>DB: 12. パスキー(公開鍵のみ)保存
    Note over DB: 秘密鍵はサーバーに<br>送信・保存されない
    サーバー->>クライアント: 13. 登録完了・ログイン情報返却
```
...
sequenceDiagram
    ...
    サーバー->>DB: 2. ユーザー確認
    Note over サーバー: 3. ランダムなデバイスID生成
    サーバー->>DB: 4. 承認リクエスト作成<br>(pending状態)
    サーバー->>新デバイス: 5. リクエストID返却
    新デバイス->>新デバイス: 6. 承認待ちページにリダイレクト<br>(/approve-wait/[requestId])
    
    loop ポーリング
        新デバイス->>サーバー: 7. ステータスポーリング<br>(/api/auth/approval/requests/[requestId]/status)
        サーバー->>DB: 8. リクエスト状態確認
        DB->>サーバー: 9. 現在のステータス
        サーバー->>新デバイス: 10. 現在のステータス返却
    end
    
    Note over 既存デバイス: 11. 通常ログイン・ダッシュボード表示
    既存デバイス->>サーバー: 12. 承認リクエスト一覧取得<br>(/api/auth/approval/requests)
    サーバー->>DB: 13. 保留中リクエスト検索
    DB->>サーバー: 14. リクエスト情報
    サーバー->>既存デバイス: 15. 保留中のリクエスト返却
    
    既存デバイス->>サーバー: 16. 承認プロセス開始<br>(/api/auth/approval/requests/[requestId]/approve/start)
    サーバー->>DB: 17. チャレンジ生成・保存
    サーバー->>既存デバイス: 18. 認証オプション返却
    
    既存デバイス->>既存デバイス: 19. WebAuthn API呼び出し<br>(navigator.credentials.get)
    Note over 既存デバイス: 20. ユーザーに認証を促す<br>(指紋/顔認証/PIN等)
    
    既存デバイス->>サーバー: 21. 認証応答送信<br>(/api/auth/approval/requests/[requestId]/approve/finish)
    サーバー->>サーバー: 22. 認証応答検証
    サーバー->>DB: 23. リクエストを承認済みに更新
    サーバー->>既存デバイス: 24. 承認成功返却
    
    Note over 新デバイス: 25. 次回ポーリングで「承認済み」検出
    Note over 新デバイス: 26. ログイン完了・ダッシュボードへリダイレクト
```
