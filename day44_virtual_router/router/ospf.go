...
// LSAInfo is a simplified version of LSA for display purposes.
type LSAInfo struct {
	...
	LinkCount           int
	AgeSeconds          int64
	RawLSA              string // For displaying raw LSA details if needed
}
...
func (oi *OSPFInstance) generateLSA() *LSA {
	log.Printf("Router %s: OSPF: generateLSA START.", oi.router.ID)

	log.Printf("Router %s: OSPF: generateLSA: Getting next sequence number.", oi.router.ID)
	currentSeqNum := oi.getNextSequenceNumber()
	log.Printf("Router %s: OSPF: generateLSA: Got sequence number %d.", oi.router.ID, currentSeqNum)
	...
	log.Printf("Router %s: OSPF: generateLSA: Initial LSA struct created (AdvRtr: %s, Seq: %d).", oi.router.ID, lsa.AdvertisingRouterID, lsa.SequenceNumber)

	// Add link for the router's own TUN network (stub network)
	log.Printf("Router %s: OSPF: generateLSA: Adding stub network link for %s.", oi.router.ID, oi.router.IPAddress.String())
	...
	log.Printf("Router %s: OSPF: generateLSA: Stub network link added.", oi.router.ID)

	// Add links for each active neighbor
	log.Printf("Router %s: OSPF: generateLSA: Attempting to call GetNeighborLinks.", oi.router.ID)
	neighborLinksSnapshot := oi.router.GetNeighborLinks() // Get a safe copy
	log.Printf("Router %s: OSPF: generateLSA: GetNeighborLinks returned, snapshot has %d links. Iterating...", oi.router.ID, len(neighborLinksSnapshot))

	for neighborID, link := range neighborLinksSnapshot {
		log.Printf("Router %s: OSPF: generateLSA: Iterating over neighbor link snapshot: Neighbor %s, Cost %d", oi.router.ID, neighborID, link.Cost)
		lsa.Links = append(lsa.Links, OSPFLinkState{
			LinkType:          "router",
	...
	log.Printf("Router %s: OSPF: generateLSA: Finished iterating neighbor links snapshot.", oi.router.ID)

	log.Printf("Router %s: OSPF: Generated LSA (Seq: %d, Links: %d). FINISHING generateLSA.", oi.router.ID, lsa.SequenceNumber, len(lsa.Links)) // Original log, good for end marker
	return lsa
}
...
// generateAndFloodLSA generates a new LSA for the current router state and floods it.
// This should be called whenever there's a change in local link state or topology.
func (oi *OSPFInstance) generateAndFloodLSA() {
	log.Printf("Router %s: OSPF: generateAndFloodLSA START.", oi.router.ID)
	newLSA := oi.generateLSA()
	if newLSA == nil {
		log.Printf("Router %s: OSPF: generateAndFloodLSA: generateLSA returned nil. Aborting.", oi.router.ID)
		return
	}
	...
	log.Printf("Router %s: OSPF: generateAndFloodLSA: Updated own LSDB with new LSA (Seq: %d).", oi.router.ID, newLSA.SequenceNumber)
	oi.lsdbMutex.Unlock()

	// Flood to neighbors
	log.Printf("Router %s: OSPF: generateAndFloodLSA: Calling floodLSA.", oi.router.ID)
	oi.floodLSA(newLSA)

	// Trigger SPF calculation because our own LSA changed
	log.Printf("Router %s: OSPF: generateAndFloodLSA: Calling triggerSPFCalculation.", oi.router.ID)
	oi.triggerSPFCalculation()
	log.Printf("Router %s: OSPF: generateAndFloodLSA FINISHED.", oi.router.ID)
}

// triggerSPFCalculation signals the OSPF process to recalculate the SPF tree.
...
// HandleLinkUp is called when a direct link to a neighbor comes up.
// It updates the OSPF state, adds the neighbor, and triggers LSA regeneration and flooding.
func (oi *OSPFInstance) HandleLinkUp(neighborID string, cost int, localIP, remoteIP net.IP) {
	log.Printf("Router %s: OSPF: HandleLinkUp START for neighbor %s (Cost: %d, LocalIP: %s, RemoteIP: %s)", oi.router.ID, neighborID, cost, localIP, remoteIP)

	oi.neighborMutex.Lock()
	// Update neighbor state if already exists, or add if new.
	// For simplicity, we assume AddNeighbor will handle new/existing logic or just add.
	// If we had neighbor states (Down, Init, TwoWay, Full), this is where we'd transition them.
	log.Printf("Router %s: OSPF: HandleLinkUp: Calling AddNeighbor for %s", oi.router.ID, neighborID)
	oi.neighborMutex.Unlock() // Unlock before calling AddNeighbor, as AddNeighbor will acquire its own lock if needed

	oi.AddNeighbor(neighborID) // This might also trigger LSA update internally if it changes link state significantly

	// Regenerate and flood LSA to reflect the new/updated link.
	log.Printf("Router %s: OSPF: HandleLinkUp: Calling generateAndFloodLSA after link up to %s.", oi.router.ID, neighborID)
	oi.generateAndFloodLSA()

	log.Printf("Router %s: OSPF: HandleLinkUp FINISHED for neighbor %s.", oi.router.ID, neighborID)
}

// HandleLinkDown is called when a direct link to a neighbor goes down.
...
// AddNeighbor adds a new neighbor to the OSPF instance or updates its state.
func (oi *OSPFInstance) AddNeighbor(neighborID string) {
	log.Printf("Router %s: OSPF: AddNeighbor START for neighbor %s.", oi.router.ID, neighborID)
	oi.neighborMutex.Lock()
	defer oi.neighborMutex.Unlock()

	if _, exists := oi.neighbors[neighborID]; !exists {
		oi.neighbors[neighborID] = &OSPFNeighborData{
			RouterID: neighborID,
			// Initialize other neighbor state fields if necessary
		}
		log.Printf("Router %s: OSPF: Added neighbor %s.", oi.router.ID, neighborID)
		// When a new neighbor is added, it might be necessary to regenerate LSA
		// if this wasn't called from HandleLinkUp (which already does it).
		// For simplicity, HandleLinkUp always calls generateAndFloodLSA.
	} else {
		log.Printf("Router %s: OSPF: Neighbor %s already exists. (No state update implemented yet).", oi.router.ID, neighborID)
	}
	log.Printf("Router %s: OSPF: AddNeighbor FINISHED for neighbor %s.", oi.router.ID, neighborID)
}

// RemoveNeighbor removes a neighbor from the OSPF instance.
...
// GetLSDBEntries returns a copy of LSAs from the LSDB for display.
func (oi *OSPFInstance) GetLSDBEntries() []LSA {
	oi.lsdbMutex.RLock()
	defer oi.lsdbMutex.RUnlock()
	entries := make([]LSA, 0, len(oi.lsdb))
	for _, entry := range oi.lsdb {
		if entry.Lsa != nil {
			entries = append(entries, *entry.Lsa) // Return a copy of LSA
		}
	}
	// Sort by AdvertisingRouterID for consistent display
	sort.Slice(entries, func(i, j int) bool {
		return entries[i].AdvertisingRouterID < entries[j].AdvertisingRouterID
	})
	return entries
}
...
func (oi *OSPFInstance) GetOSPFNeighbors() []OSPFNeighborData {
	oi.neighborMutex.RLock()
	defer oi.neighborMutex.RUnlock()
	neighbors := make([]OSPFNeighborData, 0, len(oi.neighbors))
	for _, neighbor := range oi.neighbors {
		neighbors = append(neighbors, *neighbor) // Return a copy
	}
	// Sort by RouterID for consistent display
	sort.Slice(neighbors, func(i, j int) bool {
		return neighbors[i].RouterID < neighbors[j].RouterID
	})
	return neighbors
}
